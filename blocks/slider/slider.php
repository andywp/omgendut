...
$slider->items  = $this->db->getAll("select ".$fTitle ." as title, ".$fTagline ." as tagline, ".$fDescription." as description ,image,btn_caption,url,slider_type,slider_youtube_id ,slider_order from ".$this->table_prefix."slider where slider_type='image' and publish='1' order by slider_order limit 10");
...
foreach($slider->items as $xslider){
	...
	if($xslider['slider_type']=='image'){
        ...
        /*$xgetSlider .= '
        <div class="item '.$no.$itemActive.'">
        <iframe id="video" src="https://www.youtube.com/embed/'.$xslider['slider_youtube_id'].'?enablejsapi=1" frameborder="0" allowfullscreen style="width:100%"></iframe>    
        </div>
        ';*/
		$xgetSlider .= '
        <div class="item'.$itemActive.'">
        <iframe id="video_'.$no.'" class="vid" src="https://www.youtube.com/embed/'.$xslider['slider_youtube_id'].'?enablejsapi=1" frameborder="0" allowfullscreen style="width:100%;"></iframe>    
        </div>
        ';
	}
